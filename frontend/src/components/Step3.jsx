...
const Step3 = ({ values, setFieldValue, allData }) => {
  ...
  const selectedDishes = values.dishes.filter(dish => dish.name.trim() !== '').map(dish => dish.name);
      ...
      {(values.dishes || []).map((dish, index) => (
        <div key={index} className="flex flex-row gap-8 space-y-1">
          <div className="mt-1">
            ...
            <Field as="select" name={`dishes[${index}].name`} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-48 p-2.5">
              <option value="">---</option>
              {dishChoices
